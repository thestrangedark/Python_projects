    ...
    def calc(self):
        ...
        if self.T is not None:
            ...
            if self.x<=1.0: #manual interpolation
                ...
                self.region='Superheated'
                self.T = float(griddata((hcol, pcol), tcol, (Tsat, Pbar)))  #use griddata to interpolate with h & P the superheated table
                self.s = float(griddata((hcol, pcol), scol, (Tsat, Pbar)))  #use griddata to interpolate with h & P the superheated table
        elif self.s!=None:
            ...
            if self.x<=1.0: #manual interpolation
                ...
                self.region = 'Superheated'
                self.T = float(griddata((scol, pcol), tcol, (self.s, Pbar)))  #use griddata to interpolate with s & P the superheated table
                self.h = float(griddata((tcol, pcol), hcol, (Tsat, Pbar)))  #use griddata to interpolate with s & P the superheated table
        #endregion
